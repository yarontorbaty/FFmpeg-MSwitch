...
# Minimal UDP Buffering Test
# This test uses the most aggressive UDP buffering settings for minimal latency

echo "=== Minimal UDP Buffering Test ==="
echo "This test uses minimal UDP buffering settings"
...
echo ""
echo "=== TEST 1: Minimal UDP Buffering (fifo_size=1) ==="
echo "Expected: ~0.5-1 second buffer (minimal UDP buffering)"
echo "Press Ctrl+C to stop and move to next test"
echo ""

# Minimal UDP buffering - fifo_size=1 (only 1 packet buffer!)
timeout 10s ./ffmpeg \
...
echo ""
echo "=== TEST 2: Ultra-Minimal UDP Buffering (fifo_size=0) ==="
echo "Expected: ~0.2-0.5 second buffer (ultra-minimal UDP buffering)"
echo "Press Ctrl+C to stop"
echo ""

# Ultra-minimal UDP buffering - fifo_size=0 (no circular buffer!)
timeout 10s ./ffmpeg \
  -fflags nobuffer -flags low_delay \
  -analyzeduration 0 -probesize 32 -max_delay 0 \
  -flush_packets 1 \
  ...
  -msw.enable -msw.sources "s0=local;s1=local;s2=local" \
  -msw.auto.enable -msw.auto.recovery_delay 2000 \
  -i "udp://127.0.0.1:12350?fifo_size=0&overrun_nonfatal=1&timeout=1000000&buffer_size=32768" \
  -i "udp://127.0.0.1:12351?fifo_size=0&overrun_nonfatal=1&timeout=1000000&buffer_size=32768" \
  -i "udp://127.0.0.1:12352?fifo_size=0&overrun_nonfatal=1&timeout=1000000&buffer_size=32768" \
  -filter_complex "[0:v][1:v][2:v]mswitch=inputs=3:map=0:tube=1,fps=30[out]" \
  -map "[out]" -c:v libx264 -r 30 -preset ultrafast -g 30 -keyint_min 1 -sc_threshold 0 \
  -pix_fmt yuv420p -f mpegts - | ffplay -fflags nobuffer -flags low_delay -i - -loglevel warning
