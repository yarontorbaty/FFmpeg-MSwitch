#!/bin/bash
# MSwitch Demo - Simple demonstration of MSwitch functionality
# This script shows how MSwitch works with visual feedback
...
demo_individual_streams() {
    ...
    for i in "${!streams[@]}"; do
        echo -e "${BLUE}Showing ${colors[i]} stream (${streams[i]})...${NC}"
        ...
        echo -e "${GREEN}${colors[i]} stream completed${NC}"
    done
}
...
# Main demo runner
main() {
    echo -e "${BLUE}MSwitch Demo - Visual Demonstration${NC}"
    echo -e "${BLUE}====================================${NC}"
    ...
    echo -e "\n${GREEN}MSwitch Demo completed successfully!${NC}"
