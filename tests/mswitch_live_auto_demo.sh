#!/bin/bash
# MSwitch Live Stream Auto Demo - Automated demo with visible ffplay windows
# This demo creates live UDP streams and shows MSwitch functionality automatically
...
TEST_DIR="/tmp/mswitch_live_auto_$$"
OUTPUT_PORT=12345
...
test_individual_live_streams() {
    ...
    for i in "${!streams[@]}"; do
        ...
        echo -e "${YELLOW}Watch the ffplay window for 15 seconds, then it will auto-continue${NC}"
        echo -e "${YELLOW}You should see a test pattern with audio tone ${i+1}000 Hz${NC}"
        
        # Run ffplay for 15 seconds, then auto-continue
        timeout 15 "$FFPLAY_PATH" -i "${streams[i]}" 2>/dev/null || true
...
test_live_stream_failover() {
    ...
    for i in 2 3; do
        ...
        echo -e "${YELLOW}Watch the ffplay window for 10 seconds${NC}"
        if timeout 10 "$FFPLAY_PATH" -i "udp://127.0.0.1:$port" 2>/dev/null; then
...
# Main demo runner
main() {
    echo -e "${BLUE}MSwitch Live Stream Auto Demo${NC}"
    echo -e "${BLUE}=============================${NC}"
    ...
    echo -e "\n${GREEN}MSwitch Live Stream Auto Demo completed successfully!${NC}"
    ...
    echo -e "✅ Extended visual testing with ffplay windows"
}
