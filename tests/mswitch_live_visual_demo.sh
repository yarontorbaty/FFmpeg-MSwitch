#!/bin/bash
# MSwitch Live Stream Visual Demo - Extended demo with visible ffplay windows
# This demo creates live UDP streams and shows MSwitch functionality with longer viewing time
...
TEST_DIR="/tmp/mswitch_live_visual_$$"
OUTPUT_PORT=12345
...
# Test individual live streams with longer viewing time
test_individual_live_streams() {
    echo -e "\n${CYAN}=== Test 1: Individual Live Streams (Extended Viewing) ===${NC}"
    ...
    for i in "${!streams[@]}"; do
        ...
        echo -e "${YELLOW}Watch the ffplay window for 10 seconds, then press 'q' to continue${NC}"
        echo -e "${YELLOW}You should see a test pattern with audio tone ${i+1}000 Hz${NC}"
        
        # Run ffplay for 10 seconds, then auto-continue
        timeout 10 "$FFPLAY_PATH" -i "${streams[i]}" 2>/dev/null || true
...
test_live_stream_failover() {
    ...
    for i in 2 3; do
        ...
        echo -e "${YELLOW}Watch the ffplay window for 5 seconds${NC}"
        if timeout 5 "$FFPLAY_PATH" -i "udp://127.0.0.1:$port" 2>/dev/null; then
...
interactive_live_stream_test() {
    echo -e "\n${CYAN}=== Interactive Live Stream Testing ===${NC}"
    
    echo -e "${BLUE}You can now test the live streams interactively!${NC}"
    echo -e "\n${YELLOW}Available Live Stream URLs:${NC}"
    ...
    echo -e "\n${YELLOW}MSwitch Test Command:${NC}"
    echo -e "${GREEN}./ffmpeg -msw.enable 1 -msw.sources \"s0=udp://127.0.0.1:$((OUTPUT_PORT + 1));s1=udp://127.0.0.1:$((OUTPUT_PORT + 2));s2=udp://127.0.0.1:$((OUTPUT_PORT + 3))\" -msw.ingest hot -msw.mode graceful -f lavfi -i \"testsrc=duration=1:size=320x240:rate=1\" -f null -${NC}"
    
    echo -e "\n${BLUE}Press Enter to continue with automated tests...${NC}"
    read -r
}
...
# Main demo runner
main() {
    echo -e "${BLUE}MSwitch Live Stream Visual Demo${NC}"
    echo -e "${BLUE}=================================${NC}"
    ...
    echo -e "\n${GREEN}MSwitch Live Stream Visual Demo completed successfully!${NC}"
    ...
    echo -e "✅ Extended visual testing"
}
