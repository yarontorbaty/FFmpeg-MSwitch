...
# Tube Buffering Test
# This test demonstrates the tube parameter limiting buffering to 5 frames during startup

echo "=== MSwitch Tube Buffering Test ==="
echo "This test shows how the tube parameter limits buffering during startup"
...
echo ""
echo "=== TEST 1: Default Tube Size (5 frames) ==="
echo "Expected: Minimal buffering, only 5 frames per input during startup"
echo "Press Ctrl+C to stop and move to next test"
echo ""

# MSwitch with default tube size (5 frames)
timeout 10s ./ffmpeg \
  ...
  -filter_complex "[0:v][1:v][2:v]mswitch=inputs=3:map=0:tube=5,fps=30[out]" \
...
echo ""
echo "=== TEST 2: Smaller Tube Size (2 frames) ==="
echo "Expected: Even less buffering, only 2 frames per input during startup"
echo "Press Ctrl+C to stop"
echo ""

# MSwitch with smaller tube size (2 frames)
timeout 10s ./ffmpeg \
  -fflags nobuffer -flags low_delay \
  -analyzeduration 0 -probesize 32 -max_delay 0 \
  -flush_packets 1 \
